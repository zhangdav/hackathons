...
describe("Token Vendor", function () {
    ...
    it("Should deploy YourToken", async function () {
        ...
        hpToken = await HPTokenFactory.deploy()
    })

    describe("totalSupply()", function () {
        it("Should have a total supply of at least 99999999999999999999999", async function () {
            const totalSupply = await hpToken.totalSupply()
            const totalSupplyInt = parseInt(ethers.utils.formatEther(totalSupply))
            console.log("\t", "Total Supply:", totalSupplyInt)
            expect(totalSupplyInt).to.greaterThan(9999999999999999999999)
        })
    })
    ...
    it("Should deploy Vendor", async function () {
        ...
        vendor = await VendorFactory.deploy(hpToken.address)
        console.log("Transferring 99999999999999999999999 tokens to the vendor...")
        await hpToken.transfer(vendor.address, ethers.utils.parseEther("99999999999999999999999"))
    })
    ...
    describe("buyTokens()", function () {
        it("Should let us buy tokens and our balance should go up...", async function () {
            ...
            const startingBalance = await hpToken.balanceOf(owner.address)
            ...
            const newBalance = await hpToken.balanceOf(owner.address)
            console.log("\t", "New Token balance: ", ethers.utils.formatEther(newBalance))
            expect(newBalance).to.equal(startingBalance.add(ethers.utils.parseEther("2.0")))
        })
    })
    ...
    describe("sellTokens()", function () {
        it("Should let us sell tokens and we should get the appropriate amount eth back...", async function () {
            ...
            const approveTokensResult = await hpToken.approve(
                vendor.address,
            ...
            expect(atxResult.status).to.equal(
                1,
                "Error when expecting the transaction result to equal 1",
            )
            ...
            expect(txResult.status).to.equal(
                1,
                "Error when expecting the transaction status to equal 1",
            )
    ...
    describe("onlyOwnerWithdraw()", function () {
        it("Should let the owner (and nobody else) withdraw the eth from the contract...", async function () {
            ...
            expect(buyTxResult.status).to.equal(
                1,
                "Error when expecting the transaction result to be 1",
            )

            const vendorETHBalance = await ethers.provider.getBalance(vendor.address)
            console.log(
                "\t",
                "Starting Vendor contract ETH balance: ",
            ...
            console.log(
                "\t",
                "Starting non-owner ETH balance: ",
                ethers.utils.formatEther(startingNonOwnerETHBalance),
            )

            await expect(vendor.connect(nonOwner).onlyOwnerWithdraw()).to.be.reverted
            ...
            console.log(
                "\t",
                "Starting owner ETH balance: ",
                ethers.utils.formatEther(startingOwnerETHBalance),
            )
            const withdrawResult = await vendor.onlyOwnerWithdraw()
            console.log("\t", "withdraw Result: ", withdrawResult.hash)

            console.log("\t", "Waiting for confirmation...")
            const withdrawTxResult = await withdrawResult.wait()
            expect(withdrawTxResult.status).to.equal(
                1,
                "Error when expecting the withdraw transaction to equal 1",
            )
